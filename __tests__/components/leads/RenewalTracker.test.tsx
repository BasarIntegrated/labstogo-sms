import RenewalTracker from "@/components/leads/RenewalTracker";
...
describe("RenewalTracker", () => {
  ...
  it("opens add renewal modal", () => {
    ...
    expect(screen.getByText("Add Renewal")).toBeInTheDocument();
    expect(screen.getByLabelText("Lead ID")).toBeInTheDocument();
  ...
  it("creates new renewal", async () => {
    ...
    const leadIdInput = screen.getByLabelText("Lead ID");
    ...
    fireEvent.change(leadIdInput, { target: { value: "lead3" } });
  ...
  it("edits existing renewal", async () => {
    ...
    expect(screen.getByDisplayValue("lead1")).toBeInTheDocument();
